...
import { useApp } from "../../context/AppContext"; // ✅ import context for token

export default function Refunds() {
  const { user } = useApp(); // ✅ get admin token
  ...
  useEffect(() => {
    const fetchRefunds = async () => {
      try {
        const data = await getAllRefunds(user.token);
        setRefunds(data);
      } catch (err) {
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const data = await updateRefundStatus(id, { status }, user.token);
      const updated = data.refund || data; // fallback for structure
      setRefunds((prev) =>
  ...
  const handleDeleteRefund = async (id) => {
    ...
    try {
      await deleteRefund(id, user.token);
